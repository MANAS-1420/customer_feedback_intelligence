...
def analyze_single(review_text: str) -> dict:
    try:
        ...
        # --- NEW: PSYCHOLOGICAL & BEHAVIORAL METRICS ---
        churn_risk = (subcategory in ["switching_intent", "threatening_to_leave", "trust_issue"]) or (priority_score >= 60 and final_sentiment == "negative")
        
        # Tone Intensity (1-10) based on punctuation, caps, and extreme words
        caps_ratio = sum(1 for c in raw_text if c.isupper()) / (len(raw_text) + 1)
        exclamation_count = raw_text.count('!')
        intensity = min(10.0, 2.0 + (exclamation_count * 1.5) + (caps_ratio * 20.0) + (3.0 if strong_negative else 0.0) + (2.0 if urgent else 0.0))
        
        # Sarcasm Heuristic: BERT sees negative context, but user uses strong positive words
        sarcasm_flag = final_sentiment == "negative" and any_hit(norm_text, ["great", "awesome", "perfect", "wah", "kamaal", "best"])
        ...
        return {
            ...
            "language_detected": "Unknown", "churn_risk": False, "tone_intensity_score": 0.0, "sarcasm_suspected": False, "escalation_required": False, "action_recommendation": "Skip"
        }
